# Hi there, I'm Chris! 👋

### 🎓 College Student & Aspiring Software Engineer
I am a passionate learner currently diving deep into the world of **Python**. I spend my days practicing logic, solving problems, and building the foundation for a career in **Software Engineering** and **Cloud Computing**.

---

- **Learning:** Python (Basics, Logic, Problem Solving)
- **Interest Areas:** Backend Development & Cloud Infrastructure
- **Daily Habit:** Committing code and building small Python tools

---

### 📂 Featured Projects

#### ⚡ [Appliance Cost Calculator](https://github.com)
- **Description:** A Python tool that calculates monthly electricity costs with safety buffer logic.
- **Tech Stack:** Python
- [View Code ↗](https://github.com)

#### 🐍 [Task Tracker CLI]
- **Description:** (Coming Soon! I'm working on this next.)
- **Tech Stack:** Python

---

### 🎯 2026 Goals
- [x] Create a professional GitHub Profile
- [ ] Build 5 beginner Python projects
- [ ] Learn how to use Python with Cloud APIs

---

### 📫 Connect with Me
- **GitHub:** You're already here! 🚀

